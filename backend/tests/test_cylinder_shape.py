...
def _scaled_poly_with_hole(shape: str, depth_override=None, radius=10.0):
    fh = ScaledFingerHole(
        id="fh1",
        x_mm=0.0,
        y_mm=0.0,
...
class TestCylinderShape:
    def test_cylinder_produces_cutter(self):
        poly = _scaled_poly_with_hole("cylinder", depth_override=15.0)
        config = BinParams(cutout_depth=15.0)
        result = _make_finger_holes(
            [poly], config, wall_top_z=33.0, max_depth=25.0,
            offset_x=0.0, offset_y=0.0,
        )
        assert result is not None
        bb = result.bounding_box()
        # bb is (min_x, min_y, min_z, max_x, max_y, max_z)
        # cylinder should span the full pocket depth (15mm + 0.01 epsilon)
        z_extent = bb[5] - bb[2]
    ...
    def test_cylinder_bottom_is_flat(self):
        ...
        # circle (sphere) bottom is at the same z but rounded, so the manifold
        # bbox is the same — instead, check that cylinder's bbox z_min equals
        # wall_top_z - pocket_depth (within epsilon), proving full-depth reach.
        poly = _scaled_poly_with_hole("cylinder", depth_override=12.0)
        config = BinParams(cutout_depth=12.0)
        wall_top = 30.0
        result = _make_finger_holes(
            [poly], config, wall_top_z=wall_top, max_depth=25.0,
            offset_x=0.0, offset_y=0.0,
        )
        assert result is not None
        bb = result.bounding_box()
    ...
    def test_cylinder_cuts_deeper_than_sphere_when_radius_limits(self):
        ...
        wall_top = 30.0
        depth = 15.0
        radius = 10.0  # depth (15) > radius (10) → sphere truncates

        cyl_poly = _scaled_poly_with_hole("cylinder", depth_override=depth, radius=radius)
        circ_poly = _scaled_poly_with_hole("circle", depth_override=depth, radius=radius)
        config = BinParams(cutout_depth=depth)

        cyl = _make_finger_holes([cyl_poly], config, wall_top_z=wall_top, max_depth=25.0, offset_x=0.0, offset_y=0.0)
        circ = _make_finger_holes([circ_poly], config, wall_top_z=wall_top, max_depth=25.0, offset_x=0.0, offset_y=0.0)
        cyl_floor_z = cyl.bounding_box()[2]
        circ_floor_z = circ.bounding_box()[2]

        # cylinder reaches the requested floor
        assert abs(cyl_floor_z - (wall_top - depth)) < 0.05
        # sphere only reaches `radius` below wall_top
        assert abs(circ_floor_z - (wall_top - radius)) < 0.05
        # cylinder is strictly deeper than sphere here
